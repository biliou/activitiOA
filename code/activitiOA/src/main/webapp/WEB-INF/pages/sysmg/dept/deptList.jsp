    ...
    var deptMgr = {
   		...
   		delDept:function(deptId){	
 	 		if(confirm("您确定要删除此部门吗?")){  	 			 
		 		  $.ajax({
						type:'delete',//请求方式
						url:'${ctx}/sysmg/dept/delDept/'+ deptId, 
...
<body>
	<ul class="nav nav-tabs">
		...
		<li><a href="${ctx}/sysmg/dept/gotoDeptEdit?editFlag=1">部门添加</a></li>
	</ul>

	<form id="deptListForm" method="post" action="${ctx}/sysmg/dept/gotoDeptList">
		<table id="deptTreeTable" class="table table-striped table-bordered table-condensed hide">
			...
			<c:forEach items="${deptList}" var="dept">
				<tr id="${dept.id}" pId="${dept.parentId}">										 
					<td><a href="${ctx}/sysmg/deptEdit.action?editFlag=2&deptId=${dept.id}">${dept.name}</a></td>
					<td>${dept.code}</td>
					<td style="text-align:center;">${dept.sort}</td>
					<td>${dept.master}</td>
					<td>${dept.phone}</td>	
					<td nowrap>
						<a href="${ctx}/sysmg/dept/gotoDeptEdit?editFlag=2&deptId=${dept.id}">修改</a>
						<a href="javascript:deptMgr.delDept(${dept.id})">删除</a>
						<a href="${ctx}/sysmg/dept/gotoDeptEdit?editFlag=1&parentId=${dept.id}">添加下级部门</a>
					</td>		 
				</tr>
				</c:forEach>
