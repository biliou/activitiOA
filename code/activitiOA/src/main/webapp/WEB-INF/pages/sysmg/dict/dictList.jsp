...
var dictMgr ={		 
	 	 getDictListPage :function(pageNo,pageSize){
			...
			var description = $("#description").val();
	 			$.ajax({
				type:'post',//请求方式
				url:'${ctx}/sysmg/dict/getDictListPage', 
				...
				success:function(data){//与服务器交互成功调用的回调函数
					...
					if(dictList!=null&&dictList.length>0){				
							...
							for(var i=0;i<dictList.length;i++){
								...
								htmlTable = htmlTable+"</td>";	
								htmlTable = htmlTable+"<td>";
	 			    			var editButton ="<a href='${ctx}/sysmg/dict/gotoDictEdit?editFlag=2&&dictId="+dictList[i].id+"'>修改</a>";
								var delButton = "<a href='javascript:dictMgr.delDict("+dictList[i].id+")'>删除</a>";
								var addButton = "<a href='${ctx}/sysmg/dict/gotoDictEdit?editFlag=1'>添加</a>";
	 	...
	 	delDict:function(dictId){		
	 		if(confirm("您确定要删除此字典吗?")){
		 		  $.ajax({
						type:'delete',//请求方式
						url:'${ctx}/sysmg/dict/delDict/'+dictId, 
...
<ul class="nav nav-tabs">
    ...
    <li><a href="${ctx}/sysmg/dict/gotoDictEdit?editFlag=1">字典添加</a></li>
