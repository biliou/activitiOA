    ...
    var areaMgr = {    		
   		delArea:function(areaId){	
  	 		if(confirm("您确定要删除此区域吗?")){  	 			 
  		 		  $.ajax({
  						type:'delete',//请求方式
  						url:'${ctx}/sysmg/area/delArea/' + areaId, 
...
<body>
	<ul class="nav nav-tabs">
		...
		<li><a href="${ctx}/sysmg/area/gotoAreaEdit?editFlag=1">区域添加</a></li>
	</ul>

	<form id="areaListForm" method="post" action="${ctx}/sysmg/area/gotoAreaList">
		<table id="areaTreeTable" class="table table-striped table-bordered table-condensed hide">
			...
			<c:forEach items="${areaList}" var="area">
				<tr id="${area.id}" pId="${area.parentId}">										 
					<td><a href="${ctx}/sysmg/area/gotoAreaEdit?editFlag=2&areaId=${area.id}">${area.name}</a></td>
					<td>${area.code}</td>
					<td style="text-align:center;">${area.sort}</td>
					 
					<td nowrap>
						<a href="${ctx}/sysmg/area/gotoAreaEdit?editFlag=2&areaId=${area.id}">修改</a>
						<a href="javascript:areaMgr.delArea(${area.id})">删除</a>
						<a href="${ctx}/sysmg/area/gotoAreaEdit?editFlag=1&parentId=${area.id}">添加下级区域</a>
					</td>		 
				</tr>
				</c:forEach>
