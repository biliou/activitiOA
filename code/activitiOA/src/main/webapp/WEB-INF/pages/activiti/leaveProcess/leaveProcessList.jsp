...
var leaveProcessMgr ={		 
	 getleaveProcessList:function(){
	 		loading('正在提交，请稍等...'); 		
	 			$.ajax({
				type:'post',//请求方式
				url:'${ctx}/activiti/leaveProcess/getLeaveProcessList', 
				...
				success:function(data){//与服务器交互成功调用的回调函数
					...
					if(leaveBeanList!=null&&leaveBeanList.length>0){				
							...
							for(var i=0;i<leaveBeanList.length;i++){
								...
								htmlTable = htmlTable+"</td>";
								htmlTable = htmlTable+"<td>";
	 			    			var editButton ="<a href='${ctx}/activiti/leaveProcess/gotoLeaveProcessEdit?editFlag=2&&leaveId="+leaveBeanList[i].leaveId+"'>修改</a>";
								...
								var processImageButton = "<a href='${ctx}/activiti/leaveProcess/gotoLeaveProcessImage?processInstanceId="+leaveBeanList[i].processInstanceId+"'>查看流程图</a>";
	 	...
	 	doLeaveProcess:function(leaveId){		
	 		  ...
	 		  $.ajax({
					type:'post',//请求方式
					url:'${ctx}/activiti/leaveProcess/doLeaveProcess', 
	 	...
	 	delLeaveProcess:function(leaveId){		
	 		if(confirm("您确定要删除此请假单吗?")){
		 		  $.ajax({
						type:'delete',//请求方式
						url:'${ctx}/activiti/leaveProcess/delLeaveProcess/' + leaveId, 
						dataType:'json', //有几种格式 xml html json text 等常用
						//data传值的另外一种方式 form的序列化
						//data: {"leaveId":leaveId},//传递给服务器的参数				
						success:function(data){//与服务器交互成功调用的回调函数
...
<ul class="nav nav-tabs">
    ...
    <li><a href="${ctx}/activiti/leaveProcess/gotoLeaveProcessImage">流程图</a></li>
